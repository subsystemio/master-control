...
class ConsoleModel {
  ...
  _invoke(propKey, cmd) {
    ...
    if (args.length === 0) {
      this.c.command(propKey, cmd.name, {})
      return this
    }
    this.prompt = {
      key: propKey,
  ...
  _header(w) {
    ...
    const idKey =
      paint.mute('ROOM ') +
      paint.line(c.roomId ? c.roomId.slice(0, 16) + '…' : 'joining') +
      paint.mute('   ADMIN ') +
  ...
  _commands(w) {
    const sel = this.selected()
    if (!sel)
      return panel('commands', [paint.dim('  select a program')], w, {
        colour: C.dim,
        titleColour: C.dim
      })
    ...
    const cmds = this.c.commands(sel)
    if (cmds.length === 0)
      return panel('commands', [paint.dim('  this program declared no commands')], w, {
        colour: C.dim
      })
  ...
  _footer() {
    ...
    return (
      '  ' +
      [k('↑↓', 'SELECT'), k('1-9', 'EXECUTE'), k('Q', 'DISCONNECT')].join(paint.dim('   ·   '))
    )
  }
}
