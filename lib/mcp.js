...
class MCP {
    ...
    await new Promise((resolve, reject) => {
      this._lock.on('error', (err) => {
        reject(
          err.code === 'EADDRINUSE'
            ? new Error(
                'another mcp is already serving this installation (port ' +
                  this.lockPort +
                  ').\n' +
                  '  Stop it first — two daemons on one identity split the fleet in half.'
              )
            : err
        )
      })
  ...
  _onInvoke(op, m) {
    ...
    if (!rec || !rec.channels) {
      this.log('cannot reach ' + key.slice(0, 12) + ' for ' + m.name + ' — offline')
      return op.channels.sendResult({ id: m.id, ok: false, result: '"subsystem offline"' })
    }
    if (!rec.adopted) {
      this.log('refused ' + m.name + ' to ' + rec.tag + ' — not adopted')
      return op.channels.sendResult({ id: m.id, ok: false, result: '"subsystem not adopted"' })
    }

    const id = this._nextId++
    this._pending.set(id, { operator: op, subsystemKey: key, name: m.name })
    rec.channels.sendCommand({ id, name: m.name, args: m.args })
    this.log('-> ' + rec.tag + ' ' + m.name + ' #' + id)
  }
