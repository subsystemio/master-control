...
const { identity, room, channels, attest } = require('@subsystemio/runtime')
const { Roster } = require('./roster.js')
const { Log } = require('./log.js')

const OPERATOR_HANDSHAKE_MS = 10000
...
class MCP {
  constructor({ dir, onLog, privateRoom = false, bootstrap, lockPort = LOCK_PORT } = {}) {
    ...
    this.closed = false
    this.proof = null // attestation binding this box's key to the root identity
    this.identityKey = null // the root identity, derived from the proof
    this.subsystems = new Map() // hexKey -> record
    this.operators = new Map() // hexKey -> { key, tag, role, channels }
    this._log = new Log(dir, { onLine: this.onLog })
  ...
  async start() {
    ...
    // The attestation minted offline by the root identity, binding THIS box's device key into the
    // fleet. It is what every prop checks; without it we can still run, but no prop will accept us.
    // Deliberately not fatal: you need a running MCP to read its device key before you can attest
    // it, so a first boot is legitimately unattested.
    const proofFile = path.join(this.dir, '.proof')
    this.proof = fs.existsSync(proofFile) ? fs.readFileSync(proofFile) : null
    this.identityKey = this.proof ? attest.identityOf(this.proof) : null

    if (this.proof) {
      // An attestation for a different box is worse than none: it would look configured and be
      // silently rejected by every prop. Say so here rather than in a venue.
      const attested = attest.deviceOf(this.proof)
      if (!attested || !b4a.equals(attested, this.keyPair.publicKey)) {
        throw new Error(
          '.proof attests ' +
            (attested ? b4a.toString(attested, 'hex').slice(0, 12) : '<unreadable>') +
            '… but this box is ' +
            this.pubkey.slice(0, 12) +
            '…\n' +
            '  Attest this box: mcp device, then `mcp attest <that key>` on your offline machine.'
        )
      }
      fs.writeFileSync(path.join(this.dir, '.identity-key'), b4a.toString(this.identityKey, 'hex'))
    }
    ...
    //
    // The topic follows the ROOT IDENTITY, not this box's key — that is what lets the MCP machine be
    // replaced without reflashing cards. Unattested, we fall back to our own key so a first boot
    // still forms a topic and can be found once attested.
    this.topic = topic(this.identityKey || this.keyPair.publicKey, this.roomKey)
    ...
    if (this.identityKey) {
      this.log('identity ' + b4a.toString(this.identityKey, 'hex').slice(0, 12) + '… — attested')
    } else {
      this.log('NOT ATTESTED — no prop will accept commands from this box')
      this.log('  attest it: mcp device, then `mcp attest <key>` offline, then `mcp proof <hex>`')
    }
    this.log(
      'fleet ' +
  ...
  _onSubsystem(socket) {
    ...
    // Prove which fleet we are before asking anything of the prop. It tells us nothing until this
    // verifies, so sending it first is what gets the conversation started.
    if (this.proof) ch.sendAttestation({ proof: this.proof })
  ...
  // ── upward: operators ───────────────────────────────────────────────────
  //
  // An operator is an IDENTITY, not a machine. It proves which identity this connection belongs to;
  // the roster then decides what that identity may do. So one line covers every laptop a person
  // owns, adding a machine needs no roster edit, and removing the line locks all of them out.
  //
  // Unlike the device hop we do not know the identity in advance — anyone may present one and will
  // arrive pending, exactly as an unknown key did before. What must hold is that the proof names
  // THIS peer: without that check a genuine proof belonging to an adopted operator could be
  // replayed by anyone who obtained a copy of it.
  _onOperator(socket) {
    const deviceKey = b4a.toString(socket.remotePublicKey, 'hex')
    const op = {
      key: null,
      deviceKey,
      tag: deviceKey.slice(0, 12),
      role: 'pending',
      channels: null
    }
    ...
    const onAttestation = (m) => {
      ...
      const info = attest.verify(m.proof, null, socket.remotePublicKey)
      if (!info) {
        this.log('refused operator ' + op.tag + ' — attestation does not name this peer')
        socket.destroy()
        return
      }
      identified = true
      op.key = b4a.toString(info.identityKey, 'hex')
      op.tag = op.key.slice(0, 12)
      this._admitOperator(op, socket, ch)
    }

    // Same ordering trap as the device hop: protomux can deliver the attestation *during*
    // createFleetChannels, when `ch` is still in its temporal dead zone. Buffer it — touching `ch`
    // there throws into protomux's dispatch, which destroys the stream and loops the peer forever.
    let early = null
    let ch = null
    ch = createFleetChannels(socket, {
      onAttestation: (m) => (ch ? onAttestation(m) : (early = m)),
      onInvoke: (m) => {
        if (identified) this._onInvoke(op, m)
      },
      onAdopt: (m) => {
        if (identified) this._onAdopt(op, m)
      }
    })
    if (early) onAttestation(early)

    // An operator that never identifies itself must not hold a connection open forever.
    const timer = setTimeout(() => {
      if (identified) return
      this.log('refused operator ' + op.tag + ' — never identified itself')
      socket.destroy()
    }, OPERATOR_HANDSHAKE_MS)

    socket.on('close', () => {
      clearTimeout(timer)
      if (!op.key) return
      if (this.operators.get(op.key) === op) this.operators.delete(op.key)
      this.log('operator ' + op.tag + ' detached')
    })
  }

  _admitOperator(op, socket, ch) {
    // Re-read from disk: the README says you may edit roster.txt by hand, and `mcp trust` writes to
    // it from outside this process. Loading only at startup made both silently require a restart.
    this.roster.load()
    const known = this.roster.get(op.key)

    // Bootstrap: the very first operator identity to appear on an empty roster is adopted
    // automatically, so there is always a way in. After that, adoption is an explicit act.
    if (known && known.kind === 'operator') op.role = 'admin'
    else if (this.roster.list('operator').length === 0) {
      this.roster.adopt(op.key, 'operator', 'first operator')
      op.role = 'admin'
      this.log('adopted first operator ' + op.tag + ' (roster was empty)')
    }

    op.channels = ch
    this.operators.set(op.key, op)
    ch.sendOperatorHello({ role: op.role })
    this.log(
      'operator ' +
        op.tag +
        ' attached as ' +
  ...
  // Idempotent: a console that hands its MCP off — to a restart, or to a teardown that closes
  // everything it was given — has no way to know whether someone closed it already, and the lock
  // server never calls back a second time.
  async close() {
...
function validateCaps(caps) {
  ...
  return {
    ...
    uiPort: Number.isInteger(caps.uiPort) ? caps.uiPort : 0
  }
}
